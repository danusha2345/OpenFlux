...
> ноду с `--exit-key-file`, клиента с `--peer-key`, либо передайте
> `--allow-plaintext` для незащищённого туннеля. Все клиенты (десктоп, iOS,
> Android) используют кодек **batched** и один wire-формат.
...
| `--exit-key-file` | | | Нода: файл статического ключа шифрованного транспорта (создаётся при первом запуске) |
| `--peer-key` | | | Клиент: публичный ключ выходной ноды; включает шифрование |
| `--psk-file` | | | Обе стороны, опционально: файл с общим секретом (16+ символов), закрывает ноду |
| `--allow-plaintext` | | `false` | Работа без шифрования (НЕБЕЗОПАСНО; иначе шифрование обязательно) |
